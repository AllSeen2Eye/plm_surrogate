...
def visualize_logits(seq, model, device, to_probs = False):
    ...
    n_classes = debug_dict[0][1].shape[-1]
    n_cols = 1+int(n_classes > 1)
    fig, ax = plt.subplots(len(debug_dict), n_cols, figsize = (len(debug_dict)*2, 20), width_ratios = [10, 1][:n_cols])
